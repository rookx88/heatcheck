...
// a new account (a legitimate zero-state, not a 404). Two shapes, matching how the two
// kinds are owned:
//   items - stacked consumables (food) plus persistent items (equipment, collectible -
//           both declared types with no shipped SKUs yet; the widget's Inventory
//           Collectibles tab reads them from here when they exist).
//   eggs  - one entry PER OWNED EGG, newest-first. Eggs are never quantity-stacked: each
//           is its own inventory row with its own id, which is exactly what hatch
//           consumes. Two same-color eggs are genuinely identical — same SKU, no
//           distinguishing data beyond which row they are.
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
    ...
    const rows = await sql`
        ...
        WHERE i.user_id = ${session.userId}
          AND (i.item_type = 'food' AND i.quantity > 0 OR i.item_type IN ('equipment', 'collectible'))
    ...
    return jsonResponse({ items, eggs }, { headers: authHeaders });
};
