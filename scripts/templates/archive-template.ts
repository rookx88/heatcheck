...
/**
 * Generate post card HTML for archive
 */
            ...
            <div style="display: flex; flex-direction: column; flex: 1; min-height: 0; width: 100%; box-sizing: border-box; overflow: hidden;">
                ...
                <div style="display: flex; gap: 0.5rem; margin-bottom: 0.75rem; align-items: center; justify-content: flex-start; position: relative; width: 100%; box-sizing: border-box;">
                    ...
                    <div class="heat-indicator-container" data-post-id="${post.id}" style="width: 85px; height: 85px; min-width: 85px; border: 2px solid #00ff41; border-radius: 50%; background: rgba(0, 0, 0, 0.5); display: flex; align-items: center; justify-content: center; flex-shrink: 0; position: relative; box-shadow: inset 0 0 20px #00ff4140, 0 0 15px #00ff4160; overflow: hidden;">
                        <div style="color: #00ff41; font-size: 1.2rem; font-weight: 900; -webkit-text-stroke: 2px #000000; text-stroke: 2px #000000; font-family: 'Courier New', monospace; -webkit-font-smoothing: antialiased; text-rendering: optimizeLegibility; letter-spacing: 0.5px; z-index: 1; position: relative;">DFS</div>
                    </div>
                    ` : `
                    ...
                    <div class="heat-indicator-container" data-post-id="${post.id}" style="width: 85px; height: 85px; min-width: 85px; border: 2px solid #ff0040; border-radius: 50%; background: rgba(0, 0, 0, 0.5); display: flex; align-items: center; justify-content: center; flex-shrink: 0; position: relative; box-shadow: inset 0 0 20px #ff004040, 0 0 15px #ff004060; overflow: hidden; cursor: pointer;">
                        ...
                        <div class="heat-number" style="color: #ff1a1a; font-size: 1.8rem; font-weight: 900; -webkit-text-stroke: 2px #000000; text-stroke: 2px #000000; font-family: 'Courier New', monospace; -webkit-font-smoothing: antialiased; text-rendering: optimizeLegibility; letter-spacing: 0.5px; z-index: 1; position: relative; transition: text-shadow 0.3s ease;">${heatScore}</div>
                    </div>
                    `}
...
export function generateArchivePage(
    ...
    let content = '<div class="content-area-title">▶ ARCHIVE</div>';
    content += '<div class="post-list" id="archive-list">';
    ...
    // Pagination
    if (totalPages > 1) {
        content += '<div style="margin-top: 2rem; padding: 1rem; display: flex; gap: 1rem; justify-content: center; align-items: center; color: #fff; font-family: \'Courier New\', monospace;">';
        content += `<span style="margin-right: 1rem;" id="pagination-info">PAGE ${page} / ${totalPages}</span>`;
        
        if (page > 1) {
            const prevUrl = page === 2 ? `/archive/` : `/archive/page/${page - 1}/`;
            content += `<a href="${prevUrl}" style="color: #fff; text-decoration: none; border: 1px solid #fff; padding: 0.5rem 1rem; transition: all 0.3s ease; margin-right: 0.5rem;" onmouseover="this.style.background='rgba(255, 255, 255, 0.1)';" onmouseout="this.style.background='transparent';">&lt; PREV</a>`;
        }
        
        if (page < totalPages) {
            const nextUrl = `/archive/page/${page + 1}/`;
            content += `<a href="${nextUrl}" style="color: #fff; text-decoration: none; border: 1px solid #fff; padding: 0.5rem 1rem; transition: all 0.3s ease;" onmouseover="this.style.background='rgba(255, 255, 255, 0.1)';" onmouseout="this.style.background='transparent';">NEXT &gt;</a>`;
        }
