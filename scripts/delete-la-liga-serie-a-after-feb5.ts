...
async function deleteLaLigaSerieAAfterFeb5() {
  try {
    console.log('🔍 Checking for La Liga and Serie A matchups after 2026-02-05...\n');
    ...
    const deleteResult = await pool.query(`
      ...
      WHERE league IN ('La Liga', 'Serie A')
        AND scheduled_date > '2026-02-05'
      RETURNING id
    `);

    console.log(`\n✅ Successfully deleted ${deleteResult.rows.length} matchup(s)`);
    console.log('You can now re-import La Liga and Serie A matchups with the corrected timezone conversion.');
