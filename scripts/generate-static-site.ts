...
// Character cap for the labels drawn on a Tank share card. Deliberately larger than
// tank-deck-format's in-page HEADER_LABEL_MAX_CHARS (34): the card is 1200px wide with
// 64px gutters and its header line is 19px type, which fits ~76 characters. This is a
// backstop against a freak team/player name overflowing the art, not a fit-to-width
// trim - the realistic worst case ("Bundesliga · Borussia Mönchengladbach vs. Bayern
// München", ~57) lands inside it untouched.
const CARD_LABEL_MAX_CHARS = 64;

/**
 * Write a generated binary asset (the per-article share cards) to both dist and public.
 *
 * Unlike writeHtmlFile, this writes assets/* to public too, on purpose: these files are
 * generated by the build rather than checked in, so the dev server has no other copy to
 * serve, and a card missing locally would send the local site down the fallback path and
 * hide exactly the thing being worked on.
 */
      ...
      if (LEGACY_BUILD) {
        ...
        try {
            ...
            for (const row of tankPagesResult.rows) {
                ...
                // Per-article share card (scripts/generate-og-image.ts). Built from the
                // same four labels as the article's own deck header - see
                // tank-article-template's deckPayload - so the card a stranger sees on X
                // or Discord says what the page says.
                //
                // The one difference is the cap: truncateHeaderLabel defaults to 34
                // chars because the in-page wall panel clips at a fixed width, which on
                // a 1200px card turns "New Orleans Saints vs. Detroit Lions" into
                // "NEW ORLEANS SAINTS VS.…" - the matchup, the single most valuable
                // thing on a share card, cut off mid-word. The card's header line has
                // room for roughly 76 characters, so it gets its own cap and the deck
                // keeps its 34.
                const { prop, game } = tankPage.game_snapshot;
                const cardData = {
                    league: tankPage.league,
                    contextLabel: truncateHeaderLabel(`${game.league} · ${prop.player}`, CARD_LABEL_MAX_CHARS),
                    tagline: truncateHeaderLabel(tankPage.model_output.tagline || deriveTaglineFallback(tankPage.model_output.hook), CARD_LABEL_MAX_CHARS),
                    hook: tankPage.model_output.hook,
                    sides: tankPage.model_output.call.sides,
                    oddsOrMarketLabel: truncateHeaderLabel(formatOddsLabel(prop.odds) ?? formatMarketLabel(prop.market), CARD_LABEL_MAX_CHARS),
                    settleDateLabel: truncateHeaderLabel(formatSettleDate(prop.settleDate ?? game.settleDate ?? game.kickoff), CARD_LABEL_MAX_CHARS),
                };
                ...
                // that size-capped unfurlers (WhatsApp especially) drop the preview
                // rather than show it. q85 lands the same picture in ~110KB.
                //
                // Caught per row on purpose: one bad card must never fail the build or
                // cost a reader their article. The page simply falls back to the
                // site-wide world-map image, which is what every Tank article shared
                // before this existed.
                ...
                try {
                    ...
                    cardFailures++;
                    console.warn(`⚠ Share card generation failed for ${row.slug}, falling back to the default share image:`, error.message);
                }
