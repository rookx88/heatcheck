...
1. ~~**`DAILY_PICK_CAP`'s actual deployed value.**~~ **RESOLVED (local dev + production).** Set `DAILY_PICK_CAP=3` in `.dev.vars` and re-ran the boundary suite against it — cap logic confirmed correct at 3 (picks 1–3 succeed, pick 4 gets 429/`remaining:0`, exactly 3 rows land, the `newsletter_exclusive` carve-out still doesn't count against it). Production had the same gap: the env var was never actually set in the Cloudflare Pages dashboard (Production environment), so `numEnv(context.env.DAILY_PICK_CAP, 1)` was silently falling back to 1 live. Sammy added `DAILY_PICK_CAP=3` under Production in the dashboard; since Pages Functions only pick up new env vars on the *next* deployment, triggered a redeploy via the `DEPLOY_HOOK_URL` webhook (production, branch `main`, deployment `94a2754b`) to apply it. Confirmed live on the actual site: cap is now 3. **Preview environment's value was not checked/changed** — only Production was addressed here.
